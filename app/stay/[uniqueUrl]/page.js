          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredRooms.map((room) => (
              <RoomCard 
                ...
                room={room} 
                property={property}
                onBookNow={() => router.push(`/booking/${property.uniqueUrl}/${room._id}`)}
              />
            ))}
          </div>
        )}
...
// Room Card Component
function RoomCard({ room, property, onBookNow }) {
          ...
          {property.pricing?.type === "perRoom" ? (
            ...
            <div>
              ...
              <div className="mt-1 text-xs text-gray-500">
                <div>Adults: ₹{room.adultRate || (room.perPersonPrices && room.perPersonPrices[1]) || 0} per person</div>
                <div>Children: ₹{room.childRate || Math.floor((room.perPersonPrices && room.perPersonPrices[1]) / 2) || 0} per child</div>
              </div>
            </div>
          )}
        </div>
